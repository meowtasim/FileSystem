...
#include <fstream>
#include <vector>
// #define diskSpace 10485760 // in terms of bytes
// #define blockSize 2048     // in terms of bytes
...
void createFile(directory &d, const string &fileContent, file &actualFile)
{
    ...
    {
        cout << "Made 1 block " << blocksUsed << endl;
...
void createFile2(directory &d, const string &fileLocation, file &actualFile)
{
    cout << "Starting file creation" << endl;
    ifstream file(fileLocation, ios::binary);
    std::vector<char> byteStream((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>()); // copying from file
    file.close();
    cout << " size of string is " << byteStream.size() << endl;
    int fileSize = byteStream.size(); // Calculating size of the file
    d.directorySize += fileSize;      // Update size of directory
    int blocksUsed = ceil(static_cast<double>(fileSize) / blockSize);
    actualFile.blocksUsed = blocksUsed;
    actualFile.indexTable = new int[blocksUsed]; // creates array of that many elements
    ...
    for (int k = 0; k < blocksUsed; k++)
    {
        ...
        for (i = 0; i < blockSize; i++)
        {
            FAT[blockNumber].blockPointer[i] = byteStream[i + pointer];
        }
        pointer += i;
    }
    actualFile.fileSize = sizeof(byteStream); // updating metadata
    actualFile.locationDirectory = &d;
    d.fileRecords[d.numberOfFiles] = &actualFile; // Updating directory
    d.numberOfFiles++;
...
int main()
{
    ...
    while (1)
    {
        cout << "\n1.Create File\n"
             ...
             << "12.Change owner\n"
             << "13.Read file\n"
             << "14.Create non text file\n"
             << endl;
        // cin.ignore();
        cout << "{Current Directory : " << currentDirectory->name << "} Enter Input: ";
        cin >> choice;
        switch (choice)
        {
        ...
        case 3: // Display - works
        {
            ...
            for (int i = 0; i < currentDirectory->numberOfFiles; i++)
            {
                cout << i + currentDirectory->numberOfDirectories + 1 << " : " << currentDirectory->fileRecords[i]->name << currentDirectory->fileRecords[i]->filetype << endl;
            }
            break;
        }
        ...
        case 14: // take any type of file
            if (owner == 1)
            {
                ...
                cin.ignore();
                getline(cin, fileName); // type
                f1 = new file;
                // cout<<fcontent<<endl;
                f1->filetype = fileName;
                createFile2(*currentDirectory, fcontent, *f1);
            }
            else
            {
                cout << "Unauthorized Access" << endl;
